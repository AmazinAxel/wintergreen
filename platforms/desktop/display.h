...
class DesktopEmulatorDisplay final : public wintergreen::IDisplay {
 public:
  // Desktop renders the visible area only (788px wide), skipping the 12 hidden panel columns.
  static constexpr int kPixels = wintergreen::DisplayFrame::kPhysicalWidth * wintergreen::DisplayFrame::kPhysicalHeight;
  ...
  void set_rotation(wintergreen::Rotation r) override {
    rotation_ = r;
  ...
  void full_refresh(const uint8_t* pixels, wintergreen::RefreshMode /*mode*/, bool /*turnOffScreen*/) override {
    ...
    for (int i = 0; i < kPixels; ++i) {
      const int y = i / wintergreen::DisplayFrame::kPhysicalWidth;
      const int x = i % wintergreen::DisplayFrame::kPhysicalWidth;
      const int x_buf = x + wintergreen::DisplayFrame::kPanelOffsetX;
      const std::size_t byte_idx = static_cast<std::size_t>(y * wintergreen::DisplayFrame::kStride + x_buf / 8);
  ...
  void partial_refresh(const uint8_t* new_pixels, const uint8_t* /*prev_pixels*/) override {
    ...
    for (int y = 0; y < wintergreen::DisplayFrame::kPhysicalHeight; ++y) {
      for (int x = 0; x < wintergreen::DisplayFrame::kPhysicalWidth; ++x) {
        const int x_buf = x + wintergreen::DisplayFrame::kPanelOffsetX;
        const std::size_t byte_idx = static_cast<std::size_t>(y * wintergreen::DisplayFrame::kStride + x_buf / 8);
        const uint8_t bit = static_cast<uint8_t>(0x80u >> (x_buf & 7));
        const bool new_white = (new_pixels[byte_idx] & bit) != 0;
        const bool old_white = sim_[y * wintergreen::DisplayFrame::kPhysicalWidth + x] >= 0.5f;
        if (old_white != new_white)
          sim_[y * wintergreen::DisplayFrame::kPhysicalWidth + x] = new_white ? 1.0f : 0.0f;
  ...
  void write_ram_bw(const uint8_t* data) override {
    if (gray_bw_.empty())
      gray_bw_.resize(wintergreen::DisplayFrame::kPixelBytes);
    std::memcpy(gray_bw_.data(), data, wintergreen::DisplayFrame::kPixelBytes);
  }

  // Store RED RAM data for subsequent grayscale_refresh.
  void write_ram_red(const uint8_t* data) override {
    if (gray_red_.empty())
      gray_red_.resize(wintergreen::DisplayFrame::kPixelBytes);
    std::memcpy(gray_red_.data(), data, wintergreen::DisplayFrame::kPixelBytes);
  }
  ...
  void partial_refresh_region(int phys_x, int phys_y, int phys_w, int phys_h, const uint8_t* new_buf,
    ...
    const int sim_x0 = phys_x - wintergreen::DisplayFrame::kPanelOffsetX;
    for (int row = 0; row < phys_h; ++row) {
      const int y = phys_y + row;
      if (y < 0 || y >= wintergreen::DisplayFrame::kPhysicalHeight)
        continue;
      const uint8_t* src = new_buf + row * stride_bytes;
      for (int col = 0; col < phys_w; ++col) {
        const int x = sim_x0 + col;
        if (x < 0 || x >= wintergreen::DisplayFrame::kPhysicalWidth)
          continue;
        const bool white = (src[col / 8] >> (7 - (col & 7))) & 1;
        sim_[y * wintergreen::DisplayFrame::kPhysicalWidth + x] = white ? 1.0f : 0.0f;
      }
    }
    render_();
 ...
 private:
  DesktopRuntime& rt_;
  wintergreen::Rotation rotation_ = wintergreen::Rotation::Deg0;
  ...
  void render_() {
    ...
    for (int y = 0; y < wintergreen::DisplayFrame::kPhysicalHeight; ++y) {
      ...
      for (int x = 0; x < wintergreen::DisplayFrame::kPhysicalWidth; ++x) {
        const float s = sim_[y * wintergreen::DisplayFrame::kPhysicalWidth + x];
    ...
    const bool sideways = rotation_ == wintergreen::Rotation::Deg90;
    const int win_w = sideways ? wintergreen::DisplayFrame::kPhysicalHeight : wintergreen::DisplayFrame::kPhysicalWidth;
    const int win_h = sideways ? wintergreen::DisplayFrame::kPhysicalWidth : wintergreen::DisplayFrame::kPhysicalHeight;
    SDL_Rect dst = {(win_w - wintergreen::DisplayFrame::kPhysicalWidth) / 2,
                    (win_h - wintergreen::DisplayFrame::kPhysicalHeight) / 2, wintergreen::DisplayFrame::kPhysicalWidth,
                    wintergreen::DisplayFrame::kPhysicalHeight};
